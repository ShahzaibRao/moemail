      ...
      <div className="fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] w-[90%] max-w-md">
        <div className="bg-background border-2 border-primary/20 rounded-lg p-6 md:p-12 shadow-lg">
          <div className="text-center space-y-4">
            <h1 className="text-xl md:text-2xl font-bold">Insufficient permissions</h1>
            ...
            {
              config?.adminContact && (
                <p className="text-sm md:text-base text-muted-foreground">Administrator contact information：{config.adminContact}</p>
              )
            }
            <Button 
              onClick={() => router.push("/")}
